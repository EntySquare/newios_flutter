...
class ForgetPasswordWidget extends StatefulWidget {
  ...
  ForgetPasswordWidget({Key key, @required this.parameter}) : super(key: key);

  @override
  _ForgetPasswordWidgetState createState() =>
      _ForgetPasswordWidgetState(this.parameter);
}
...
class _ForgetPasswordWidgetState extends State<ForgetPasswordWidget> {
  ...
  _ForgetPasswordWidgetState(parameter) {
  ...
  void getPhoneCode() {
    showDialog(
      ...
      if (err != null) {
        //获取验证失败
        Toast.toast(context, msg: "获取验证失败", position: ToastPostion.center);
      } else {
        reGetCountdown();
      }
    });
  ...
  _netRigister() async {
    // Navigator.pop(context);
    try {
      Response response = await Dio().post(url + 'findPwd',
          data: {"phone": this._phone, "pwd": this._firstPassword},
          options: Options(
